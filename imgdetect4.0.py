...
class DetectorApp:
    ...
    def setup_ui(self):
        ...
        # 【关键修改】使用 tk.Frame 而不是 ttk.Frame，并保存引用
        # 这样才能支持背景颜色的动态修改 (.config(bg=...))
        self.main_frame = tk.Frame(self.root, padx=15, pady=15, bg=self.default_bg)
        self.main_frame.pack(fill="both", expand=True)

        # --- 1. 监控目标区域 ---
        # 注意：所有子控件的父容器现在是 self.main_frame
        group_img = ttk.LabelFrame(self.main_frame, text=" 1. 监控目标 ", padding=10)
        ...
        # --- 2. 警报设置 ---
        group_audio = ttk.LabelFrame(self.main_frame, text=" 2. 警报设置 ", padding=10)
        ...
        # --- 3. 运行参数 ---
        group_settings = ttk.LabelFrame(self.main_frame, text=" 3. 运行参数 ", padding=10)
        ...
        # --- 4. 底部控制 ---
        self.btn_control = tk.Button(self.main_frame, text="启动监控", command=self.toggle_detection,
    ...
    def loop_detection(self):
        with mss.mss() as sct:
            ...
            # 定义一个函数同时改变 root 和 main_frame 的颜色
            ...
            while self.is_running:
                ...
                try:
                    ...
                    if max_val >= threshold:
                        ...
                        # 【关键修改】同时对 root 和 main_frame 闪烁红色
                        self.root.after(0, lambda: set_window_color("#ff3333")) #R
                        time.sleep(0.3) 
                        self.root.after(0, lambda: set_window_color("#3333ff")) # B
                        time.sleep(0.3)
                        self.root.after(0, lambda: set_window_color("#33ff33")) # G
                        time.sleep(0.3)
