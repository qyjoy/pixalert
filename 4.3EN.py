...
class DetectorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PixAlert - V4.3 Alive") 
        ...
        self.btn_preview_text = tk.StringVar(value="👁 Hide Preview") 
    ...
    def setup_ui(self):
        ...
        # --- 1. Target Monitor ---
        group_img = ttk.LabelFrame(self.main_frame, text=" 1. Target Monitor ", padding=10)
        ...
        ttk.Button(btn_frame, text="📂 Import", command=self.load_image_file).pack(side="left", fill="x", expand=True, padx=2)
        ...
        self.lbl_preview = tk.Label(self.preview_container, text="Please snip or import a target image", bg="#f0f0f0", fg="#888")
        self.lbl_preview.pack(fill="both", expand=True)

        # --- 2. Actions ---
        group_audio = ttk.LabelFrame(self.main_frame, text=" 2. Actions ", padding=10)
        ...
        chk_sound = ttk.Checkbutton(action_switch_row, text="Found Sound", variable=self.enable_sound)
        chk_sound.pack(side="left", padx=(0, 10))
        
        chk_click = ttk.Checkbutton(action_switch_row, text="Auto Click", variable=self.enable_click)
        ...
        self.lbl_audio_name = ttk.Label(audio_row, text="Found Audio: Default", width=25, anchor="w")
        self.lbl_audio_name.pack(side="left")
        ttk.Button(audio_row, text="Select Found...", command=self.load_success_audio, width=15).pack(side="right")

        self.fail_audio_row = ttk.Frame(group_audio)
        self.fail_audio_row.pack(fill="x", pady=(5,0))
        self.lbl_fail_audio_name = ttk.Label(
            self.fail_audio_row, text="Fail Audio: Default", width=25, anchor="w"
        )
        self.lbl_fail_audio_name.pack(side="left")
        ttk.Button(
            self.fail_audio_row, text="Select Fail...", command=self.load_fail_audio, width=15
        ).pack(side="right")
        ...
        ttk.Radiobutton(mode_row, text="Count:", variable=self.play_mode, value="times").pack(side="left")
        ...
        ttk.Radiobutton(mode_row, text="Duration(s):", variable=self.play_mode, value="duration").pack(side="left")
        ttk.Spinbox(mode_row, from_=1, to=60, width=5, textvariable=self.play_value).pack(side="left", padx=5)

        # --- 3. Parameters ---
        group_settings = ttk.LabelFrame(self.main_frame, text=" 3. Parameters ", padding=10)
        group_settings.pack(fill="x", pady=5)

        f_row = ttk.Frame(group_settings)
        f_row.pack(fill="x", pady=5)
        ttk.Label(f_row, text="Interval (s):", width=12).pack(side="left")
        ...
        ttk.Label(c_row, text="Strictness:", width=12).pack(side="left")
        ...
        # --- Log Box (New) ---
        group_log = ttk.LabelFrame(self.main_frame, text=" Event Log ", padding=10)
        ...
        # --- Control ---
        self.btn_control = tk.Button(self.main_frame, text="Start Monitor", command=self.toggle_detection,
                                     bg="#007bff", fg="white", font=("Microsoft YaHei UI", 14, "bold"),
                                     relief="flat", height=2)
        self.btn_control.pack(fill="x", pady=(10, 0))

        self.status_bar = tk.Label(self.root, text="Ready", bd=1, relief="sunken", anchor="w", bg="#e9ecef", padx=5, font=("Arial", 9))
    ...
    def set_target_image(self, img):
        ...
        self.last_detect_time = None
        self.status_bar.config(text=f"Target Loaded: {img.shape[1]}x{img.shape[0]} px", bg="#e9ecef")
    ...
    def update_preview_visibility(self):
        if self.target_image is None:
            self.btn_preview_text.set("👁 Hide Preview")
            return

        if self.is_preview_hidden:
            self.lbl_preview.config(image="", text="[ Preview Hidden ]\n\nPrevents self-detection errors", fg="#666")
            self.btn_preview_text.set("👁 Show Preview")
        else:
    ...
    def load_success_audio(self):
        ...
        if path:
            ...
            self.lbl_audio_name.config(text=f"Found Audio: {name}")
            self.log_msg(f"Found Audio set to: {name}")
    ...
    def load_fail_audio(self):
        ...
        if path:
            ...
            name = os.path.basename(path)
            if len(name) > 20: name = name[:17] + "..."
            self.lbl_fail_audio_name.config(text=f"Fail Audio: {name}")
            self.log_msg(f"Fail Audio set to: {name}")

    def toggle_detection(self):
        if self.is_running:
            self.is_running = False
            self.btn_control.config(text="Start Monitor", bg="#007bff")
            self.status_bar.config(text="Monitor Stopped", bg="#e9ecef")
            self.log_msg("Monitor Stopped.")
            pygame.mixer.music.stop()
        else:
            if self.target_image is None:
                messagebox.showwarning("Error", "Please snip or import a target image first!")
                return
            
            self.is_running = True
            self.btn_control.config(text="🛑 Stop Running", bg="#dc3545")
            self.log_msg("Monitor Started.")
    ...
    def perform_click(self, monitor, match_loc, target_shape):
        try:
            ...
            self.log_msg(f"Clicked at: {screen_x}, {screen_y}")
            ...
            while self.is_running:
                ...
                try:
                    ...
                    if max_val >= threshold:
                        ...
                        msg = f"Target Found! Conf:{max_val:.1%} @ Pos({max_loc[0]}, {max_loc[1]})"
                        ...
                        msg = f"Searching... (Max Conf: {max_val:.1%})"
                        if self.last_detect_time:
                            msg += f" | Last found: {self.last_detect_time}"
