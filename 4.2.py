...
class DetectorApp:
    def __init__(self, root):
        self.root = root
        self.root.title("PixAlert - Treasure Hunter V4.1 EN") 
    ...
    def setup_ui(self):
        ...
        chk_sound = ttk.Checkbutton(action_switch_row, text="Play Sound", variable=self.enable_sound)
        chk_sound.pack(side="left", padx=(0, 20))
        
        chk_click = ttk.Checkbutton(action_switch_row, text="Auto Click", variable=self.enable_click)
        chk_click.pack(side="left")

        audio_row = ttk.Frame(group_audio)
        audio_row.pack(fill="x")
        self.lbl_audio_name = ttk.Label(audio_row, text="Current: System Default", width=30, anchor="w")
        self.lbl_audio_name.pack(side="left")
        ttk.Button(audio_row, text="Select Audio...", command=self.load_audio_file, width=12).pack(side="right")

        mode_row = ttk.Frame(group_audio)
        mode_row.pack(fill="x", pady=(5, 0))
        ttk.Radiobutton(mode_row, text="Play Count:", variable=self.play_mode, value="times").pack(side="left")
        ...
        self.status_bar = tk.Label(self.root, text="Ready - Waiting for command", bd=1, relief="sunken", anchor="w", bg="#e9ecef", padx=5, font=("Arial", 9))
    ...
    def load_audio_file(self):
        ...
        if path:
            ...
            self.lbl_audio_name.config(text=f"Current: {name}")
